...
@Component({
  ...
  <div class="tabs">
    <ul class="tab-group">
      <li class="tab" [class.active]="tab.active" *ngFor="let tab of tabs; let i=index"
      (click)="setActive(i)">
    ...
    <div class="tab-content-group">
      ...
      // ngIf 가 false 가 되면 아예 DOM을 만들지 않는다. display: none 하고는 다름.
      </ng-container>
    </div>
  </div>
  `,
...
export class AppComponent {
  setActive(i: number) {
    this.tabs.map((tab, index) => tab.active = i === index ? true : false);
    // this.tabs = this.tabs.map((tab, index) => i === index ? { ...tab, active: true } : { ...tab, active: false });
  }
  tabs = [
    {
      title: 'HTML',
