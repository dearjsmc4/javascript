...
(function () {
  ...
  function removeTodo(id) {
    fetchTodo('DELETE', `http://localhost:9000/todos/${id}`);
  }

  $todos.onchange = function (e) {
    const id = +e.target.parentNode.id;
    fetchTodo('PATCH', `http://localhost:9000/todos/${id}`, {
  ...
  $inputTodo.onkeyup = function (e) {
    ...
    fetchTodo('POST', 'http://localhost:9000/todos', {
  ...
  $completeAll.onchange = function (e) {
    if (e.target.checked) {
      fetchTodo('PATCH', 'http://localhost:9000/todos', {
        completed: true
      });
    } else {
      fetchTodo('PATCH', 'http://localhost:9000/todos', {
  ...
  $clearCompleted.onclick = function () {
    fetchTodo('DELETE', 'http://localhost:9000/todos/completed');
  };
  ...
  $nav.onclick = function (e) {
    ...
    navId = e.target.id;
    fetchTodo('GET', 'http://localhost:9000/todos');
  };
